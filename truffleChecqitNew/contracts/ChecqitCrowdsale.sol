...
contract Crowdsale is Ownable{
  ...
  // preIco constants
  uint public preIcoStart = 0; //21.02.2018 1519189260
  uint public preIcoFinish = 1521694740; //21.03.2018

  uint public preIcoMaxCap = 60000000*pow(10,decimals); //60000000
  ...
  uint icoMinCap = 25000000*pow(10,decimals);
  ...
  function() public payable{
    ...
    addContributor(msg.sender,msg.value,now);

    // require(buy(msg.sender,msg.value,now)); //redirect to func buy
  }

  //ICO cabinets functions
  ...
  mas[] private contributors;
 
  function showContributors() public view returns (mas[]){
    return contributors;
  }

  event NewConributor(uint id, address indexed _address, uint indexed _value, uint indexed _time);
  

  function addContributor(address _address, uint _value, uint _time) internal {
    mas memory newConriburor;
    newConriburor._address = _address;
    newConriburor._value = _value;
    newConriburor._time = _time;
    newConriburor._processed = false;

    contributors.push(newConriburor);
    
    NewConributor(contributors.length, _address, _value, _time);
  }

  function confirmPayment(uint i) public onlyOwner {
    require (!contributors[i]._processed);

    require(buy(contributors[i]._address,contributors[i]._value,contributors[i]._time));
    contributors[i]._processed = true;

  }

  function rejectPayment(uint i) public onlyOwner {
    require (!contributors[i]._processed);   
    contributors[i]._address.transfer(contributors[i]._value);
    contributors[i]._processed = true;
  } 
}